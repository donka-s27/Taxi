...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([tableView isEqual:tableViewController.tableView]) {
        MKMapItem *dataForRowAtIndexPath = [data objectAtIndex:indexPath.row];
        [cell setBackgroundColor:[UIColor clearColor]];
        [[cell textLabel] setText:dataForRowAtIndexPath.name];
        cell.textLabel.font = self.font;
        [[cell detailTextLabel] setText:dataForRowAtIndexPath.placemark.country];
        cell.textLabel.textColor = self.textColor;
        cell.detailTextLabel.font = [UIFont fontWithName:self.font.familyName size:10];
...
{
    selectedData= [[self applyFilterWithSearchQuery:self.text] objectAtIndex:indexPath.row];
    self.text = selectedData.name;
...
- (NSArray *)applyFilterWithSearchQuery:(NSString *)filter
{
    ...
    NSArray *filteredGoods = [NSArray arrayWithArray:data];
...
- (void)provideSuggestions
{
    ...
    if (tableViewController.tableView.superview == nil && [[self applyFilterWithSearchQuery:self.text] count] > 0) {
        ...
        tableViewController.tableView.scrollEnabled = YES;
        ...
        [[self superview]bringSubviewToFront:tableViewController.tableView];
    }
    else{
